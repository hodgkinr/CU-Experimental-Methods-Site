...
`[NEEDS INSTRUCTOR REVIEW: Decide whether these internal CSVs should remain only as planning-side reference artifacts or be removed from the canonical compilation later.]`
...
## Missing External Data Resources

`[INSTRUCTOR TO PROVIDE: Final SharePoint location for student-facing data access.]`

`[MISSING_DATA: If the teaching team wants a public worked example, provide a single curated sample dataset through SharePoint rather than bundling raw files into the site build.]`
