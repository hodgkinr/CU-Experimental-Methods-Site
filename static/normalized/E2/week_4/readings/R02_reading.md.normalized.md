...
Read this if two or more input uncertainties come from the same source. Common examples include sensor outputs derived from the same calibration procedure or parameters estimated from the same material tests. Sampling them independently ignores that structure. The effect on output uncertainty depends on the covariance sign and on the signs and magnitudes of the model sensitivities. Correlation can widen or narrow the output distribution.
...
*This image makes input correlation visible so you can see exactly what gets lost when a Monte Carlo model samples coupled variables as if they were independent.
...
Geometrically, the Cholesky factor stretches and rotates the independent standard normal cloud into the correct correlated shape. It does not change the marginal distributions. Each input still has its correct individual uncertainty, but the samples are linked so the joint behavior matches the physical relationship between the sources.
