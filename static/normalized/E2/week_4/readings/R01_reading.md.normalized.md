...
<image description: A two-panel figure. Left panel: a scatter plot of measured data (circles) with two fitted curves — a solid line labeled "Model A: correct functional form, R² = 0.82" that tracks the data with small random scatter around it, and a dashed line labeled "Model B: polynomial fit, R² = 0.97" that tracks the data more tightly. Both curves appear close to the data. Right panel: two residual plots stacked vertically, one for Model A (random scatter around zero, small amplitude) and one for Model B (systematic S-curve trend around zero, similar amplitude). A caption reads: "R² favors Model B. Residuals reveal that Model A has better physical structure." Engineering plot style, clean black and white with labeled axes.]>

---

## Confidence Intervals: What "95%" Actually Means

**Practical rule:** Use confidence intervals for comparison (do these two results overlap?) rather than for declaring probability about a single result.
...
What this means practically is that confidence intervals must be interpreted in terms of the estimation process, not the individual result. When comparing two results — say, the mean drag coefficient measured at two different angles of attack — the comparison of confidence intervals tells you whether the two results are statistically distinguishable. If the intervals overlap substantially, you cannot confidently claim the results differ. If they do not overlap, the difference is unlikely to be sampling noise.
