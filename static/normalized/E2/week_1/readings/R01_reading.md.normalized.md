...
<image description: A side-by-side illustration contrasting genuine prediction and postdiction. On the left, a timeline arrow runs left to right: "Model developed" → "Experiment run" → "Data compared to prediction." A bold vertical line separates the model from the data, labeled "No data crosses this line." On the right, the same timeline but reversed: "Data collected" → "Model fit to data" → "Fit presented as validation." A double-headed arrow loops between the data and the model, labeled "Model adjusted until fit is good." Both panels show the same scatter plot of data and the same fitted curve — visually identical results — but one is prediction and one is postdiction. The contrast makes the invisible structural difference visible. Clean infographic style with black and gold color scheme.]>
