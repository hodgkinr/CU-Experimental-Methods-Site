...
Safety planning is sometimes treated as an administrative step that happens after the real engineering is done. That is backwards. A physical experiment changes the world on purpose: it applies loads, moves parts, heats surfaces, stores energy, spins motors, pressurizes fluids, powers circuits, or asks people to work near equipment that can fail. A basic **experimental safety plan** is the document that says what could go wrong, how likely it is, how serious it would be, and what controls make the test acceptable.

<image description: A planned lab setup shown as a clean engineering drawing with hazard callouts: rotating propeller, hot surface, electrical power supply, suspended mass, sharp specimen failure, and trip hazard from cables. Each callout connects to a safety-plan table row for hazard, consequence, likelihood, mitigation, and emergency response.>

The first section is **hazard identification**. A hazard is a source of possible harm, not merely a thing that feels dangerous. Stored electrical energy, rotating hardware, pressurized containers, hot components, suspended weights, brittle fracture, sharp edges, chemical exposure, laser light, and loud noise are all examples. For E3, you may not build the experiment, but your design still needs to show that you can recognize the hazards your proposed setup would create.
...
<image description: A hierarchy-of-controls pyramid adapted for an aerospace teaching lab. From top to bottom: eliminate hazard, substitute lower-energy test, engineering control, administrative control, PPE. Example icons include replacing a high-speed propeller test with a guarded low-speed test, adding a shield, using a checklist, and wearing eye protection.>
...
Approval and sign-off close the loop. In professional environments, a test may require review by a lab manager, safety officer, principal investigator, or responsible engineer before it runs. In this course, the instructor or TA plays that role. Your paper design should make their review possible by naming hazards, controls, operating limits, and stop conditions clearly enough that a responsible person could decide whether the experiment is acceptable.
