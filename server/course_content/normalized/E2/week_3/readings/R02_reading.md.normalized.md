...
![A two-panel quantization diagram generated from one shared sensor-response model and actual 12-bit ADC quantization. The left panel shows the same response inside a ±10 V range with a correctly tiny quantization step; the right panel shows it inside a ±200 mV range, where the same converter assigns far more codes to the response.](../images/E2_W7_R2_image1_updated.png)
...
This example requires knowing that some sensors count events as unsigned integers (always positive, values 0 to 65,535) while many DAQ systems record data as signed integers (positive and negative). A raw unsigned counter rollover and a signed-integer decoding boundary are related encoding issues, but they occur at different code values.

Consider a counter-based displacement sensor that outputs a count value from 0 to 65,535 (a 16-bit unsigned integer) representing a full rotation. The sensor counts up as the shaft rotates in one direction. When the physical count advances from 65,535 to 65,536, the 16-bit sensor word rolls over from 65,535 to 0. If a DAQ system decodes those raw words as signed 16-bit integers, a separate representation boundary occurs at 32,768: the raw word 32,768 is displayed as −32,768, and the raw word 65,535 is displayed as −1. The raw unsigned rollover remains a 65,535 → 0 discontinuity; neither discontinuity represents a reversal of the physical shaft.
...
![A two-panel time-series plot generated from a constant-acceleration count model. The upper panel shows the continuous unwrapped physical count alongside the recorded unsigned 16-bit counter, which wraps exactly from 65,535 to 0. The lower panel shows the separate signed-int16 decoding boundary at 32,768, making clear that neither jump is a physical reversal.](../images/E2_W7_R2_image2_updated.png)
