...
A film camera captures still frames at a fixed rate, typically 24 per second. If the blades complete almost exactly one full rotation between frames, they appear stationary in the footage. If they complete slightly more than one full rotation, they appear to spin backward. The blades are moving at full speed. The camera samples the scene too slowly relative to the rotation rate, so it produces an artifact that looks like real motion.

This optical illusion is more than a curiosity. It is a physical analogy for one of the most consequential pitfalls in digital data acquisition: **aliasing**.

---

Every digital measurement system works the same way the film camera does: every data acquisition (DAQ) board, every microcontroller input, and every data logger. It samples a continuous physical signal at discrete moments in time. Between samples, it knows nothing. The physical signal could be doing anything, and the measurement system will never know.

The Nyquist-Shannon sampling theorem formalizes the helicopter illusion. The same artifact appears in accelerometer data from a vibration test when the sample rate is too low. A 380 Hz structural mode can appear in your data file as a 120 Hz signal, with nothing in the data to tell you it is wrong.

To faithfully capture a signal with frequency content up to some maximum frequency *f_max*, you must sample at a rate of at least *2f_max*. This threshold is the **Nyquist rate**. Sample faster than this and you can reconstruct the original signal. Sample slower and you cannot. In practice, engineers usually sample well above the minimum, often 5 to 10 times the highest frequency of interest, because real filters and real signals are not ideal.
...
That sounds simple. In practice, it requires real knowledge of the physics. A structural vibration test on a beam might have fundamental modes below 100 Hz and no relevant content above 1 kHz. A 2,500 samples-per-second rate would be conservative and appropriate. A combustion pressure transient in a rocket engine test might contain critical content up to 50 kHz. That same 2,500 samples-per-second rate would alias the 50 kHz content catastrophically, folding it back into the sampled range below the 1.25 kHz Nyquist frequency and producing clean-looking data that is entirely fictitious.

When you commit to a sampling rate, you are making a claim. You are stating, before you collect data, that your signal contains no frequency content above half your chosen sampling rate. If that claim is wrong, and the signal has higher-frequency energy you did not anticipate, the sampled data are silently corrupted. No error message. No obvious artifact. Just a dataset that reflects physical content into data that do not exist.
...
One important consequence: you cannot fix aliased data after the fact with a software filter. Once the physical signal has been sampled too slowly and high-frequency content has folded into the measured band, the corrupted low-frequency artifact is already in the data. Software filters are useful after data collection, but anti-aliasing protection has to happen before sampling.
